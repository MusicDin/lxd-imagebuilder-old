...
func (s *debootstrap) Run() error {
	...
	if distro == "ubuntu" && lxdShared.ValueInSlice(release, []string{"xenial", "bionic"}) || distro == "debian" && lxdShared.ValueInSlice(release, []string{"sid"}) || distro == "mint" && lxdShared.ValueInSlice(release, []string{"tara", "tessa", "tina", "tricia", "ulyana"}) || distro == "devuan" {
	...
	if !lxdShared.PathExists(scriptPath) && s.definition.Source.SameAs != "" {
